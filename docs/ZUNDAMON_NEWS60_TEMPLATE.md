...
This routine prepares the repeatable YMM4 path without creating a video. The detailed one-time setup, four reusable YMM4 item layouts, and per-episode import package are in `docs/YMM4_NEWS60_ROUTINE.md`. The local baseline project is created and checked in YMM4 on Windows; no unvalidated `.ymmp` is generated by the helper.
...
Copy `examples/ymm4_news60_script_template.json`, fill its null fields and source mapping, then run the preferred package command:

    python scripts/prepare_ymm4_news60_routine.py --input path/to/script.json --output-dir path/to/new-episode-ymm4-inputs

The package includes the import CSV, review cues, beat sheet, per-run instructions, and hash manifest. It refuses an existing output directory. For the older two-file-only route, run:
