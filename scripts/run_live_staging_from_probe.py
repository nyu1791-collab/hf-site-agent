#!/usr/bin/env python3
"""Start bounded live Executor/Reviewer staging from redacted evidence.

Capability admission may be satisfied by fresh reused/deferred evidence. Only
actual network capability calls are charged to the command budget. This module
never changes production routing or writes repository source.
"""
...
from scripts.provider_adapters import NVIDIA_DEEPSEEK_MODEL, NVIDIA_NEMOTRON_MODEL, create_provider_adapter
...
def _candidate_ok(evidence: Mapping[str, Any], probe: Mapping[str, Any], provider: str, model: str) -> bool:
    ...
    return (
        ...
        and _fresh(record)
        and (record.get("zero_cost_verified") is True or (provider != "google" and record.get("staging_probe_allowed") is True))
...
def _limited_nvidia_candidate_ok(evidence: Mapping[str, Any], probe: Mapping[str, Any], model: str) -> bool:
    provider = "nvidia"
    record = _model_record(evidence, provider, model)
    probe_record = _probe_record(probe, provider, model)
    ...
    return (
        ...
        and (not probe_record.get("response_model") or probe_record.get("response_model") == model)
...
def _capability_policy(provider: str, model: str, family: str, record: Mapping[str, Any]) -> ExecutionPolicy:
    return ExecutionPolicy(
        ...
        circuit_closed=True, paid_fallback=False, auto_top_up=False, max_retries=0,
...
def run_from_reports(
    ...
    *,
    ...
    if not executor_choice or not reviewer_choice:
        selected_limited_model = next((candidate for candidate in NVIDIA_LIMITED_BOOTSTRAP_MODELS if _limited_nvidia_candidate_ok(evidence, probe, candidate)), None)
        if allow_limited_nvidia_bootstrap and selected_limited_model:
            ...
            policy = ExecutionPolicy(
                scope="STAGING", provider_id="nvidia", model_id=model,
                model_family=MODEL_FAMILIES[("nvidia", model)], technically_ready=False,
                staging_approved=True, exact_model_verified=record.get("model_verified") is True,
                endpoint_verified=record.get("endpoint_verified") is True,
                auth_verified=record.get("auth_verified") is True, capability_verified=False,
                free_verified=False, cost_safe=False, quota_safe=record.get("quota_safe") is True,
                circuit_closed=True, paid_fallback=False, auto_top_up=False, max_retries=0,
            ...
            binding = LiveAgentBinding("EXECUTOR", "nvidia", model, MODEL_FAMILIES[("nvidia", model)], adapter, policy)
            plan = build_nvidia_limited_bootstrap_plan(
                mission_id="phase9-nvidia-google-bootstrap", request_budget=1, token_budget=1_024,
                objective="Review the Google provider adapter and propose one minimal, low-risk change that can move Google toward a fail-closed staging probe. Return a proposal only.",
            )
            report = run_nvidia_limited_bootstrap_mission(
                plan, binding, ledger_path=ledger_path, checkpoint_root=checkpoint_root, network_enabled=True,
            )
            report["nvidia_limited_staging"] = {
                ...
                "probe_consumed": True, "request_limit": 1,
            }
            report.update(executor_model=model, executor_family=MODEL_FAMILIES[("nvidia", model)], reviewer_model="work-integrator-local", reviewer_family="LOCAL", production_active=False)
            report["self_bootstrap"] = {"started": False, "status": "WAITING_FOR_TWO_AGENT_FAMILY", "proposal_review_passed": False, "work_integration_required": True, "external_model_calls": 0}
            report["total_external_model_calls_in_command"] = int((report.get("live_staging") or {}).get("external_model_calls", 0))
            return report
        return {
            "status": "blocked", "stop_reason": "TWO_FRESH_ZERO_COST_MODEL_FAMILIES_REQUIRED",
            "live_staging": False, "live_provider_count": 0, "live_model_family_count": 0,
            "model_calls": 0, "paid_execution_count": 0, "paid_fallback_count": 0, "production_active": False,
        }
    ...
    for role, choice in (("EXECUTOR", executor_choice), ("REVIEWER", reviewer_choice)):
        ...
        try:
            raw_capability = adapter.capability_probe(model, "structured_output")
            capability = raw_capability if isinstance(raw_capability, Mapping) else {}
            capability_status = str(capability.get("status") or "CAPABILITY_FAILED")
            # Reused/deferred capability evidence explicitly declares that no
            # network call occurred. Legacy/real probes default to one call.
            if capability.get("network_call") is not False:
                capability_network_calls += 1
        except Exception:
        ...
        if capability_status != "CAPABILITY_OK":
            return {
                "status": "blocked", "stop_reason": "CAPABILITY_BENCHMARK_FAILED",
                "capability_results": capability_results,
                "capability_network_calls": capability_network_calls,
                "live_staging": False, "model_calls": capability_network_calls,
                "paid_execution_count": 0, "paid_fallback_count": 0, "production_active": False,
            }
        policy = _capability_policy(provider, model, MODEL_FAMILIES[choice], record)
        bindings.append(LiveAgentBinding(role, provider, model, MODEL_FAMILIES[choice], adapter, policy))

    executor, reviewer = bindings
    plan = build_minimal_staging_plan(
        mission_id="phase9-live-executor-reviewer", executor=executor, request_budget=6, token_budget=2_048,
    )
    command_request_budget = int(plan.max_total_requests)
    report = run_live_staging_mission(
        plan, executor, reviewer,
        ledger_path=ledger_path, checkpoint_root=checkpoint_root,
    ...
    report["capability_network_calls"] = capability_network_calls
    ...
    remaining_calls = max(0, command_request_budget - capability_network_calls - first_calls)
    bootstrap: dict[str, Any] = {
        "started": False, "status": "NOT_STARTED", "proposal_review_passed": False,
        "work_integration_required": True, "external_model_calls": 0,
    }
    if report.get("status") == "completed" and remaining_calls > 0:
        bootstrap_plan = build_minimal_staging_plan(
            mission_id="phase9-self-bootstrap", executor=executor,
            request_budget=remaining_calls, token_budget=2_048,
            task_id="SELF-BOOTSTRAP-1", task_role="SELF_BOOTSTRAP_PROPOSAL",
            objective="Analyze the current AI Army implementation and propose one small, low-risk regression test or adapter-contract improvement. Return a proposal only; do not edit the repository or weaken any safety guard.",
        )
        ...
        bootstrap_report = run_live_staging_mission(
            bootstrap_plan, executor, reviewer,
            ledger_path=bootstrap_ledger, checkpoint_root=Path(checkpoint_root) / "self-bootstrap",
            bounds=AutonomousBounds(max_iterations=3, max_revisions=2, max_replans=1, max_requests=remaining_calls, max_tokens=2_048),
            network_enabled=True,
        )
    ...
    report["command_request_budget"] = command_request_budget
    report["total_external_model_calls_in_command"] = capability_network_calls + first_calls + int(bootstrap.get("external_model_calls", 0))
    return report
...
def main() -> int:
    ...
    parser.add_argument("--allow-limited-nvidia-bootstrap", action="store_true", help="permit one post-probe NVIDIA staging bootstrap proposal")
    ...
    try:
        ...
        report = run_from_reports(
            evidence, probe, network_enabled=args.network,
            ledger_path=args.ledger, checkpoint_root=args.checkpoint_root,
            allow_limited_nvidia_bootstrap=args.allow_limited_nvidia_bootstrap,
        )
    except Exception:
        report = {
            "status": "blocked", "stop_reason": "LIVE_STAGING_INPUT_INVALID", "live_staging": False,
            "model_calls": 0, "paid_execution_count": 0, "paid_fallback_count": 0, "production_active": False,
        }
    ...
    print(json.dumps({
        "status": report.get("status"), "live_staging": report.get("live_staging", False),
        "model_calls": report.get("model_calls", 0), "production_active": report.get("production_active", False),
    }, sort_keys=True))
    return 0
